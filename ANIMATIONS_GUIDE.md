...
7. [Шаг 4. Shared Element Transitions (аналог matchedGeometryEffect)](#шаг-4-shared-element-transitions-аналог-matchedgeometryeffect)
    - [4.4. Компактная карточка ↔ развёрнутая (без навигации)](#44-компактная-карточка--развёрнутая-без-навигации)
...
13. [Шаг 9.5. Подводные камни Next.js App Router + View Transitions API](#шаг-95-подводные-камни-nextjs-app-router--view-transitions-api)
14. [Анти-паттерны](#анти-паттерны)
15. [Чек-лист готовности](#чек-лист-готовности)
...
### 4.4. Компактная карточка ↔ развёрнутая (без навигации)

Отдельный подвид shared-element transition — раскрытие виджета **внутри одной страницы** без смены маршрута. Классический кейс: на дашборде лежит маленькая карточка со статой, при тапе она плавно разворачивается на весь экран и показывает график крупнее, подробности, контролы. SwiftUI-аналог — `matchedGeometryEffect` между двумя ветками условного рендера.

В отличие от Шага 5 (FLIP), этот приём не требует ни `useLayoutEffect`, ни ручного считывания `DOMRect` — всё делает View Transitions API. В отличие от Шага 4 (list → detail через навигацию), здесь нет `router.push` и роуты не меняются.

#### Правило № 1: одновременно в DOM только одна версия

Это критично. Если compact и expanded рендерятся параллельно и у обоих стоит `view-transition-name: primary-card`, браузер видит дубль имени → выбрасывает `InvalidStateError` и падает в no-op. Решение — условный рендер по булевому флагу:

```tsx
{!expanded && <CompactCard onClick={toggleCard} />}
{expanded  && <ExpandedOverlay onClose={toggleCard} />}
```

У обоих контейнеров **одно и то же** `view-transition-name` в style, но в DOM они никогда не пересекаются.

#### Правило № 2: `flushSync` обязателен

`setState` сам по себе батчится. Внутри `startViewTransition` callback React может не успеть закоммитить новый DOM до того, как браузер снимет «новый» снапшот. Оборачиваем в `flushSync` из `react-dom`:
...
export function useExpandableCard() {
  ...
  const toggle = useCallback(() => {
    ...
    // Fallback для Firefox/старых Safari: просто переключаем стейт
...
#### Правило № 3: один класс на обе версии

Chrome/Safari интерполируют между старым и новым снапшотом геометрию (`width`, `height`, `top`, `left`, `border-radius`, `transform`) и делают cross-fade содержимого. Если у compact и expanded **разные CSS-классы** с принципиально разным `background`, `box-shadow`, `padding` — морф выглядит как «подмена». Лучший UX получается, когда оба состояния наследуют общий визуальный базис:

```tsx
// Compact
<section
  className="primary-card rounded-[28px] p-5"
  style={{ viewTransitionName: 'primary-card' }}
  onClick={toggle}
>
  <Header />
  <Chart />
...
// Expanded
<div className="primary-overlay" onClick={toggle}>
  <div
    className="primary-card primary-card--expanded rounded-[32px] p-7"
    style={{ viewTransitionName: 'primary-card' }}
...
Общий класс `.primary-card` держит `background`, `box-shadow`, `border` — а модификатор `--expanded` подправляет только размеры и паддинги. Фон морфится как единое целое.

#### Правило № 4: `stopPropagation` на интерактивных детях

Compact-контейнер сам реагирует на клик (он и разворачивает карточку). Если внутри у него есть кнопки (переключатели периода, иконка в углу), их клики всплывут до контейнера и тоже запустят expand. Каждой такой кнопке нужен `e.stopPropagation()`:
...
В expanded-версии наоборот — клик по backdrop-у должен закрывать, а клик по самой карточке — нет. Поэтому backdrop ловит close, а у контейнера карточки стоит `onClick={(e) => e.stopPropagation()}`.

#### Правило № 5: тюнинг pseudo-элементов

Дефолтный морф группы — это `::view-transition-group(root)` с линейной 0.25s анимацией. Для крупного элемента вроде полноэкранной карточки это мало и дёргано. Переопределяем:

```css
::view-transition-group(primary-card) {
  animation-duration: var(--dur-slow);      /* 480ms */
  animation-timing-function: var(--ease-spring);
}
::view-transition-old(primary-card),
::view-transition-new(primary-card) {
  animation-duration: var(--dur-slow);
  animation-timing-function: var(--ease-out-quart);
}
```

Группа (геометрия) едет пружиной `--ease-spring`, содержимое cross-fade-ится гладкой `--ease-out-quart` — ровно как в iOS, где `matchedGeometryEffect` использует `interactiveSpring`.

#### Правило № 6: блокировка скролла и Escape

Expanded-версия — это модальный оверлей. UX-минимум:
...
// Body не скроллится под оверлеем
useEffect(() => {
  if (!expanded) return;
...
#### Правило № 7: переиспользуемый стейт между версиями

Если у карточки есть внутренние контролы (например, переключатель Day/Week/Month), их стейт должен жить **выше** обеих версий — в компоненте-родителе. Тогда при морфе выбранный таб сохраняется, и пользователь не видит «сброса» при сворачивании.

```tsx
const [range, setRange] = useState<Range>('Day');
// и в compact, и в expanded — один и тот же range
```

#### Чек-лист для compact ↔ expanded

- [ ] Две версии рендерятся условно (`!expanded` / `expanded`), никогда одновременно.
- [ ] У обеих версий одинаковое `view-transition-name` на контейнере.
- [ ] Переключение обёрнуто в `startViewTransition(() => flushSync(setState))`.
- [ ] Есть fallback для браузеров без API (`typeof document.startViewTransition !== 'function'`).
- [ ] Внутренние кнопки в compact-версии делают `e.stopPropagation()`.
- [ ] Expanded-версия закрывается по backdrop/Escape, body-скролл заблокирован.
- [ ] `::view-transition-group(<name>)` настроен под нужную длительность и easing.
- [ ] Общий класс у обеих версий — фон и скруг­ление морфятся непрерывно.
- [ ] Состояние внутренних контролов (табы, фильтры) живёт в родителе.

> **Когда использовать FLIP (Шаг 5), а не этот приём:** если нужно поддерживать Firefox без флагов или Safari < 17.5. View Transitions API работает только в Chromium 111+ и Safari 17.5+. Иначе — этот приём проще, лаконичнее и не требует считывать `DOMRect` вручную.
...
| Картинка перерисовывается заново | `priority` + `next/image` + **строго одинаковый src** на обеих страницах |
| `view-transition-name` пропадает между снапшотами | Назначать имя через `style`, а не через CSS-класс с условием |

#### 7.2.1. Картинка пропадает в момент морфа (next/image)

**Симптом:** при переходе list → detail карточка-источник на миг становится «дырой» (фоном контейнера), и только потом появляется hero. Морф визуально «ломается» в самой ответственной точке.

**Причина:** карточка и hero используют **разные URL** одного изображения (например `?w=800` для карточки и `?w=1600` для hero). Для `next/image` это два разных источника → два разных запроса в `/_next/image?url=...&w=...` → два разных файла в кэше браузера. Когда View Transitions API делает снапшот «после», новый `<img>` ещё не загрузил байты, и в момент морфа отрисовывается пусто.

То же происходит, если у `<Image>` в карточке и в hero **разные `sizes`** — Next.js сгенерирует разные элементы `srcset`, и браузер выберет разные файлы.

**Решение:** один URL и один `sizes` на обеих страницах. Картинку показывайте через CSS-размер контейнера, а не через разные URL.

```ts
// src/data/places.ts — БЫЛО (плохо)
export const cardImage = (seed: string) => `https://cdn/${seed}/800/600`;
export const heroImage = (seed: string) => `https://cdn/${seed}/1600/1200`;

// СТАЛО (хорошо) — один URL для обоих
const SHARED = (seed: string) => `https://cdn/${seed}/1200/900`;
export const cardImage = SHARED;
export const heroImage = SHARED;
```

```tsx
// PlaceCard.tsx
<Image
  src={cardImage(place.seed)}
  alt={place.title}
  fill
  sizes="(max-width: 480px) 100vw, 480px"   // <-- одинаковый sizes
  className="object-cover"
/>

// place/[id]/page.tsx
<Image
  src={heroImage(place.seed)}                // <-- та же функция
  alt={place.title}
  fill
  sizes="(max-width: 480px) 100vw, 480px"   // <-- такой же sizes
  priority                                    // <-- обязательно
  className="object-cover"
/>
```

Картинка станет чуть «тяжелее» в карточке (1200px вместо 800), но для 6–10 карточек на мобильном это копейки, а морф становится бесшовным.

**Дополнительная страховка** — фон-плейсхолдер того же оттенка, что доминирует на картинке: даже если кэш не сработает, дыра не будет белой.
...
> **Правило:** если два `<Image>` должны морфиться через View Transitions, у них **обязательно** должны совпадать `src`, `sizes` и (для статичных изображений) `quality`. Это прямой пункт из чек-листа §9.
...
## Шаг 9.5. Подводные камни Next.js App Router + View Transitions API

Гайд из §3–§4 в чистом виде **не запустит морф** в реальном Next.js-проекте. Вот три гарантированные проблемы и их фиксы.

### 9.5.1. `@view-transition { navigation: auto }` не срабатывает на soft-навигации

`<Link>` в Next.js делает SPA-переход через React Router — браузер не перезагружает документ, поэтому нативный CSS-триггер `@view-transition { navigation: auto }` (который завязан на полные навигации) **молчит**. Кросс-фейд root'а не запускается, и View Transitions API остаётся «спать».

**Решение:** перехватывать клик и оборачивать `router.push()` в `document.startViewTransition()` вручную.
...
export function PlaceCard({ place }: { place: Place }) {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    document.startViewTransition(async () => {
      router.push(href);
      await waitForElement(`[data-place-detail="${place.id}"]`);
    });
  };
...
### 9.5.2. `requestAnimationFrame` внутри `startViewTransition` callback'а **никогда не сработает**

Самая коварная ловушка. Стандартный совет «дождаться двух `requestAnimationFrame`, чтобы React закоммитил» здесь не работает: пока callback `startViewTransition` не зарезолвился, **браузер ставит рендер на паузу**, кадры не производятся, `rAF` не вызывается. Через ~4 секунды промис `ready` падает с `TimeoutError: View transition update callback timed out`.

```ts
// ❌ ПЛОХО — повиснет на 4 секунды и упадёт по таймауту
document.startViewTransition(async () => {
  router.push(href);
...
**Решение:** ждать через `MutationObserver` — он триггерится на DOM-коммит React'а независимо от кадров.

```ts
// ✅ ХОРОШО — резолвится в момент коммита, обычно за 5–30 мс
...
document.startViewTransition(async () => {
  router.push(href);
  await waitForElement(`[data-place-detail="${id}"]`);
});
```

Чтобы наблюдатель мог по чему-то опознать «новая страница приехала» — поставьте на корневой элемент детальной страницы маркер:

```tsx
// app/place/[id]/page.tsx
<article data-place-detail={place.id}>...</article>
```

### 9.5.3. `InvalidStateError: Transition was aborted because of invalid state`

Если на странице-списке у **всех** карточек постоянно проставлен `view-transition-name`, при soft-навигации в DOM на короткий момент будут одновременно жить и старая карточка-источник, и новый hero с **одним и тем же именем**. Спецификация требует уникальности → Chrome/Safari ломают транзицию.

**Решение:** ставить `view-transition-name` **только на активной карточке** — той, по которой кликнули. На всех остальных карточках имени быть не должно.
    ...
    <>
      {places.map((p) => (
        <PlaceCard
          ...
          onActivate={() => {
            // flushSync — синхронно ставим имя в DOM ДО startViewTransition,
            // чтобы снапшот «до» уже его содержал
            flushSync(() => setActiveId(p.id));
          }}
        />
      ))}
    </>
...
function PlaceCard({ place, isActive, onActivate }: Props) {
  // Имя живёт в DOM только когда карточка активна
  const vtName = isActive
    ? { viewTransitionName: `place-image-${place.id}` }
    : undefined;

  return <div style={vtName}>...</div>;
}
```

Для **обратной** навигации (detail → list) тот же приём через `sessionStorage`: на детальной перед `router.back()` сохраняем `id`, на списке читаем его в `useState(initial)` и сразу подставляем `activeId`. Тогда снапшот «после» на возврате уже содержит карточку с нужным именем.

```ts
// PlacesList.tsx
const [activeId, setActiveId] = useState<string | null>(() => {
  if (typeof window === 'undefined') return null;
  return window.sessionStorage.getItem('__active_place__');
});
```

### 9.5.4. Сводная памятка «почему не работает морф в Next.js»

| Симптом | Корень | Фикс |
|---|---|---|
| Просто кросс-фейд страницы, нет морфа | soft-нав не триггерит `@view-transition` | `startViewTransition(() => router.push(href))` |
| `TimeoutError` через ~4 с после клика | `rAF` спит во время VT-callback'а | Ждать через `MutationObserver`, не `rAF` |
| `InvalidStateError: invalid state` | дубликаты `view-transition-name` в обоих снапшотах | `view-transition-name` только на активной карточке |
| Картинка пропадает в момент морфа | разные `src`/`sizes` у `<Image>` карточки и hero | один URL, один `sizes`, `priority` на hero (см. §7.2.1) |
| Морф работает, но с задержкой ~800 мс в dev | Next.js dev-mode компилит роут на лету | Делать `router.prefetch(href)` в `useEffect` карточки **+** проверять в `next start` (prod) |
