...
полноширинная кнопка «Добавить в избранное». Передаёт `highlightQuery`
из `?q=`. Mobile и desktop (`DesktopRestaurantDetail`) рендерятся siblings
через `md:hidden` / `hidden md:flex` — `display: none` скрытого варианта
исключает его из снимка VT API, коллизии VT-name между mobile- и
desktop-hero нет.
